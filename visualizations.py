...
def plot_model_comparison(model_results, output_path=None):
    """
    Tworzy wykres porównujący dokładność różnych modeli.
    
    Args:
        model_results: Słownik z wynikami modeli {nazwa_modelu: {accuracy: wartość, ...}}
    ...
    # Dodaj wartości dokładności na słupkach
    ...
    plt.ylim(0.5, 1.0)  # Zakres osi Y od 0.5 do 1.0 dla lepszej widoczności różnic
...
def plot_confusion_matrices(model_results, y_test, predictions, output_path=None):
    """
    Tworzy wykres macierzy pomyłek dla wszystkich modeli.
    
    Args:
        model_results: Słownik z wynikami modeli
        y_test: Rzeczywiste wartości dla zbioru testowego
        predictions: Słownik z przewidywaniami modeli {nazwa_modelu: predykcje}
        output_path: Ścieżka do zapisania wykresu (opcjonalnie)
    """
    # Liczba modeli
    ...
    if n_models == 1:
        axes = [axes]  # Zapewnienie, że axes jest listą nawet dla jednego modelu
    
    # Dla każdego modelu narysuj macierz pomyłek
    for i, (name, y_pred) in enumerate(predictions.items()):
        cm = confusion_matrix(y_test, y_pred)
        
        # Oblicz wartości dla anotacji (procenty)
...
def plot_feature_importance(feature_names, importances, model_name, output_path=None):
    """
    Tworzy wykres ważności cech dla danego modelu.
    
    Args:
        feature_names: Lista nazw cech
        importances: Lista lub tablica wartości ważności cech
